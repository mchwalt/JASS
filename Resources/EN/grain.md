...
SAMPLER itself may stay off, and both may sound together.
...
- **DENS** — grains per second, from single clicks to a dense pad. 32 grains sound at once;
  beyond that the extra ones are simply not started.
...
In the MOD MATRIX, POS, SIZE, PITCH, AMP and PAN are targets — PITCH there shifts the **centre** of
the cloud: CHAOS X → POS wanders through the recording, an LFO → PITCH with QUANT on walks a
scale. A texture generator, not a pitch-shifter — for clean transposition use SAMPLER + STRETCH.
