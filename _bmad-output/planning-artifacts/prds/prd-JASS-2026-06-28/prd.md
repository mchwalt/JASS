...
_(First sanctioned audio/DSP change. The oscillator cross-modulation operands become user-selectable now that modules are freely arrangeable.)_
- **FR21.** The oscillator-coupling module (**CROSS MOD**, formerly "MIX MODE") applies **RingMod** or **FM** to **two user-selectable oscillators** (Source A, Source B ∈ {OSC 1, OSC 2, OSC 3}). FM: A modulates B (carrier); RingMod: A × B; the remaining oscillator is summed plainly. **The mode is only RingMod / FM — "Additive" is not a mode; "no coupling" = the module disabled** (the enable toggle), matching the Filter/LFO enable+combo pattern; default = disabled ⇒ plain additive sum (identical to prior default). Source A and B are always distinct (auto-kept). New source params are append-only; the on-disk `.synthy` keeps the `Additive/RingMod/FM` marker via `choiceOrOff`, so older presets round-trip. Scope is the three OSCs only (Noise/Wavetable/Sub are a separate future idea).
