...
- **Format versioning + explicit factory baseline on load (user-requested 2026-07-11).** `kFormatVersion` bumped **1 → 2** (layout era). `applyVar` now reads `FormatVersion` (absent ⇒ 1) and makes the load **factory-reset-first explicit**: reset every param to default **and** clear the `rackLayout` property to factory, *then* layer the file's values on top. So older presets (no version / missing fields / no `RackLayout`) load safely with factory defaults for whatever they omit, and no stale state leaks from the previously-loaded patch. The version number is reserved for future *value* migrations (a field whose meaning changed) — plain new fields stay handled by missing⇒default. C# interop deprioritized (per user); a v2 file is still field-compatible.
- **Per-module factory visibility + factory reset (user-clarified 2026-07-11).** Model: preset stores everything (sound + layout); **load applies the stored visibility exactly** (a module visible in the preset shows even if it was hidden in the workspace; one not visible is hidden+silent) — already the behaviour. New: `ModuleDescriptor::defaultVisible` (default true; false ⇒ module starts hidden so the rack doesn't overflow). **Factory RESET** (header) now also resets the layout: `resetBtn` calls `processor.resetToDefault()` **+** `Rack::resetLayout()`, restoring factory zones/order/**visibility** and silencing factory-hidden modules. `resetLayout()` now runs `enforceHiddenDisabled()`; `isDefaultLayout()` compares `visible` against the per-module default (not "all visible"). All modules currently `defaultVisible=true` (appearance unchanged) — which ones start hidden is a pending content decision. C# interop deprioritized for now (per user), format stays append-only regardless.
